...
```mermaid
graph TD
    Choice["Final Choice in Ruins"] --> EndingA["Ending A: The Protector
(Seal Restored + Document Returned)"]
    Choice --> EndingB["Ending B: The Expedient
(Document Returned + Seal Stolen)"]
    Choice --> EndingC["Ending C: The Bargain
(Peace with Sembiyan + Shared Escape)"]
    Choice --> EndingD["Ending D: Costly Victory
(Guardian Slain in Combat)"]
    Choice --> EndingE["Ending E: Catastrophic Collapse
(Sanctum Destroyed / Retreat)"]
```
